...
#define MAX_ALLOCABLE_BYTES 2097152ULL /* 2 MB */
#endif
...
void * init_run(){
	...
	while(*start==0);
#if KERNEL_BD == 0
	ops[myid] = BM_ALLOCATIONS;
	bimodal(fixed_size, max_size, allocs+myid, failures+myid, frees+myid);
#else	
	unsigned long nodemask = 0x01;
	set_mempolicy(MPOL_BIND, &nodemask,sizeof(unsigned long));
	syscall(NR_BIMODAL,fixed_order, max_order, allocs+myid, failures+myid, frees+myid);
#endif
	pthread_exit(NULL);
}
...
int main(int argc, char**argv){
  printf("USING ALLOCATOR: %s\n", ALLOCATOR_NAME);
	...
	max_size = MAX_ALLOCABLE_BYTES;
	max_order = convert_to_level(MAX_ALLOCABLE_BYTES);
